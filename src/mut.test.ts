...
import { forM, forMVoid } from "./type-class/traversable.js";
...
test("doubling", () => {
    ...
    expect(pow([1, 1, 1, 0])(44)).toStrictEqual([
        1134903170, 701408733, 701408733, 433494437,
    ]);
});

test("reading dropped ref", () => {
    expect(() => {
        doMut(<S>(cat: MutCat<S>) =>
            cat
                .addM("ref", newMutRef(42))
...
test("functor laws", () => {
    ...
    doMut(<S>(cat: MutCat<S>) => {
        ...
        return cat
            .addM(
                "samples",
                forM(traversable, cat.monad)(range(-100, 100))((i) =>
                    f.map((x: number) => x)(cat.monad.pure(i)),
                ),
            )
            .finishM(({ samples }) =>
                forMVoid(
                    traversable,
                    cat.monad,
                )(zip(samples)(range(-100, 100)))(([sample, source]) => {
                    expect(sample).toStrictEqual(source);
    ...
    doMut(<S>(cat: MutCat<S>) => {
        ...
        return cat
            ...
            .addM(
                "right",
                forM(traversable, cat.monad)(range(-100, 100))((i) =>
                    f.map(plus3)(f.map(double)(cat.monad.pure(i))),
                ),
            )
            .finishM(({ left, right }) =>
                forMVoid(traversable, cat.monad)(zip(left)(right))(([l, r]) => {
...
test("applicative functor laws", () => {
    ...
    doMut(<S>(cat: MutCat<S>) => {
        ...
        return cat
            .addM(
                "samples",
                forM(traversable, cat.monad)(range(-100, 100))((x) =>
                    a.apply(a.pure((i: number) => i))(a.pure(x)),
                ),
            )
            .finishM(({ samples }) =>
                forMVoid(
                    traversable,
                    cat.monad,
                )(zip(samples)(range(-100, 100)))(([sample, x]) => {
                    expect(sample).toStrictEqual(x);
                    return cat.monad.pure([]);
                }),
            );
    });
    ...
    doMut(<S>(cat: MutCat<S>) => {
        ...
        return cat
            ...
            .addM(
                "right",
                forM(traversable, cat.monad)(range(-100, 100))((x: number) =>
                    a.apply(add3)(a.apply(sq)(cat.monad.pure(x))),
                ),
            )
            .finishM(({ left, right }) =>
                forMVoid(traversable, cat.monad)(zip(left)(right))(([l, r]) => {
    ...
    doMut(<S>(cat: MutCat<S>) => {
        ...
        return cat
            .addM(
                "left",
                forM(traversable, cat.monad)(range(-100, 100))((x) =>
                    a.apply(a.pure(double))(cat.monad.pure(x)),
                ),
            )
            .addM(
                "right",
                forM(traversable, cat.monad)(range(-100, 100))((x: number) =>
                    cat.monad.pure(double(x)),
                ),
            )
            .finishM(({ left, right }) =>
                forMVoid(traversable, cat.monad)(zip(left)(right))(([l, r]) => {
    ...
    doMut(<S>(cat: MutCat<S>) =>
        cat
            .addM(
                "left",
                forM(traversable, cat.monad)(range(-100, 100))((i) =>
                    cat.monad.flatMap(cat.monad.pure)(cat.monad.pure(i)),
                ),
            )
            .addM(
                "right",
